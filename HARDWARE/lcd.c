...
//LCD的画笔颜色和背景色	     RRRR RGGG GGGB BBBB
u16 POINT_COLOR=0x0000;	//画笔颜色   565
...
{ 					
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_FSMC,ENABLE);	//使能FSMC时钟
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOB|RCC_APB2Periph_GPIOD|RCC_APB2Periph_GPIOE|RCC_APB2Periph_GPIOG,ENABLE);//使能PORTB,D,E,G以及AFIO复用功能时钟
 	...
 	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8;				 //PG8 推挽输出 背光
...
{  							  
  uint8_t temp,t1;
	uint16_t t;
	uint16_t y0=y;
	uint16_t csize=(size/8+((size%8)?1:0))*size;		//得到字体一个汉字对应点阵集所占的字节数	
	for(t=0;t<csize;t++)
	{   
		if(size==16) temp=Chinese_16x16[(num-1)*32+t]; 	 	//调用1206字体
		
		else if(size==32)temp=Chinese_32x32[(num-1)*128+t];	//调用1608字体
//		else if(size==64)temp=Chinese_64x64[(num-1)*512+t];	//调用2412字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80) LCD_Fast_DrawPoint(x,y,POINT_COLOR);
